...
function render_details(data)
{
  ...
  else
  {
    ...
    console.log('.emojis-details');
    $('.emojis-details').hide();


      // $('.details .name').html('<h1 id="iconid" onclick="selectText(\'iconid\');">'+data.id+'<\/h1>');
    $('.selected-icon').val(`fa-${data.id}`);
    console.log(2,$('.selected-icon').val());
